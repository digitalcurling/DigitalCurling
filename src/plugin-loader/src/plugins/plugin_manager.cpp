...
const std::string LibraryExtension = DIGITALCURLING_LIBRARY_EXTENSION;
    ...
    void RegisterStaticPlugin(PluginInfo info, PluginApi api) {
        auto resource = digitalcurling::plugins::PluginResource::Create(info, api);
...
std::optional<PluginId> PluginManager::LoadPlugin(const std::filesystem::path& plugin_path, bool throw_exception) {
    ModulePtr handle = nullptr;
    std::string errorHeader = "\"" + plugin_path.string() + "\": ";

    auto exit = [&](const std::string& msg) -> std::optional<PluginId> {
        if (!throw_exception) return std::nullopt;
        else throw plugin_error { DIGITALCURLING_ERR_FAILED_TO_LOAD_PLUGIN, errorHeader + msg };
    };
    ...
    try {
        handle = LoadSharedLibrary(plugin_path);
        auto funcs = GetPluginExportedFunctions(handle);
        if (!funcs.get_plugin_info || !funcs.get_plugin_api) {
            return exit("Plugin is missing the entry point functions.");
        }

        auto info = funcs.get_plugin_info();
        ...
        auto api = funcs.get_plugin_api();
        auto resource = PluginResource::Create(info, api, std::move(handle));
        RegisterPlugin(std::move(resource));
        return PluginId{ static_cast<PluginType>(info.plugin_type), info.plugin_name };
...
void PluginManager::RegisterPlugin(std::shared_ptr<PluginResource> resource) {
    std::unique_lock<std::shared_mutex> lock(mutex_);

    auto type = resource->GetType();
    if (type == PluginType::player) {
        auto player_resource = std::dynamic_pointer_cast<PlayerPluginResource>(resource);
        if (player_resource) {
            player_resources_[resource->GetName()] = std::move(player_resource);
            return;
        }
    } else if (type == PluginType::simulator) {
        auto simulator_resource = std::dynamic_pointer_cast<SimulatorPluginResource>(resource);
