...
PluginResource::PluginResource(PluginInfo info, PluginApi api, std::optional<ModulePtr> handle)
...
std::shared_ptr<PluginResource> PluginResource::Create(PluginInfo info, PluginApi api, std::optional<ModulePtr> handle) {
    if (info.plugin_type == DIGITALCURLING_PLUGIN_TYPE_SIMULATOR) {
        if (api.simulator) return std::make_shared<SimulatorPluginResource>(std::move(info), std::move(api), std::move(handle));
        throw plugin_error{DIGITALCURLING_ERR_INVALID_PLUGIN_TYPE, "PluginResource: Plugin type is SIMULATOR but SimulatorApi is missing."}  ;
    } else if (info.plugin_type == DIGITALCURLING_PLUGIN_TYPE_PLAYER) {
        if (api.player) return std::make_shared<PlayerPluginResource>(std::move(info), std::move(api), std::move(handle));
        throw plugin_error{DIGITALCURLING_ERR_INVALID_PLUGIN_TYPE, "PluginResource: Plugin type is PLAYER but PlayerApi is missing."};
    } else {
        throw plugin_error{DIGITALCURLING_ERR_INVALID_PLUGIN_TYPE, "Plugin reported an unknown or invalid plugin type."};
    }
}

PlayerPluginResource::PlayerPluginResource(PluginInfo info, PluginApi api, std::optional<ModulePtr> handle)
    : PluginResource(std::move(info), api, std::move(handle)),
...
SimulatorPluginResource::SimulatorPluginResource(PluginInfo info, PluginApi api, std::optional<ModulePtr> handle)
    : PluginResource(std::move(info), api, std::move(handle)),
