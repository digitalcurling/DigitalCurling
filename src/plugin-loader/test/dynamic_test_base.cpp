...
void digitalcurling::test::DynamicTestBase::SetUp() {
    ...
    if (sim_res.has_value()) {
        EXPECT_TRUE(false) << "Warning (Setup): Failed to load simulator plugin: " << sim_res.value() << std::endl;
    } else {
    ...
    if (player_res.has_value()) {
        EXPECT_TRUE(false) << "Warning (Setup): Failed to load player plugin: " << player_res.value() << std::endl;
    } else {
